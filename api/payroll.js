...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      let q = supabase.from('payroll').select('*').order('month', { ascending: false }).order('staff_name');
      if (req.query.month) q = q.eq('month', req.query.month);
      if (req.query.staff_id) q = q.eq('staff_id', req.query.staff_id);
